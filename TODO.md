...
- [x] **Боты массово регистрируются** — добавлена трёхуровневая защита регистрации:
  honeypot-поле `website` (CSS-скрыт, отклоняет если заполнен), rate limit 5 попыток/час по IP
  (`FileBasedCache`), блокировка одноразовых email-доменов (`_DISPOSABLE_EMAIL_DOMAINS`).
...
| ~~Защита регистрации от ботов~~ | ✅ Выполнено |
